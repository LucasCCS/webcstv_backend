<!-- invl-call-to-action-teste -->
<section class="invl-call-to-action-teste">
    <div class="container">
        <div class="invl-call-to-action-teste-header">
            <div class="row justify-content-between align-items-center">
                <div class="col-sm-12 col-md-12 col-lg-8">
                    <h2>Acesso aos<br>melhores canais</h2>
                </div>
                <div class="col-sm-12 col-md-12 col-lg-4 text-center">
                    <a class="btn btn-default btn-block" href="<?=base_url();?>cadastro">Teste Gratuito</a>
                    <small>Teste e comprove toda a qualidade!</small>
                </div>
            </div>
        </div>
        <div class="invl-call-to-action-teste-body">
            <div class="invl-call-to-action-teste-channels-list">
                <div class="row" slick-channels>
                    <div class="col-sm-12 col-md-12 col-lg-3">
                        <div class="invl-call-to-action-teste-channels-list-item">
                            <img class="img-fluid" src="public/images/channel-01.png">
                        </div>
                    </div>
                    <div class="col-sm-12 col-md-12 col-lg-3">
                        <div class="invl-call-to-action-teste-channels-list-item">
                            <img class="img-fluid" src="public/images/channel-02.png">
                        </div>
                    </div>
                    <div class="col-sm-12 col-md-12 col-lg-3">
                        <div class="invl-call-to-action-teste-channels-list-item">
                            <img class="img-fluid" src="public/images/channel-03.png">
                        </div>
                    </div>                   
                    <div class="col-sm-12 col-md-12 col-lg-3">
                        <div class="invl-call-to-action-teste-channels-list-item">
                            <img class="img-fluid" src="public/images/channel-05.png">
                        </div>
                    </div>
                    <div class="col-sm-12 col-md-12 col-lg-3">
                        <div class="invl-call-to-action-teste-channels-list-item">
                            <img class="img-fluid" src="public/images/channel-06.png">
                        </div>
                    </div>
                    <div class="col-sm-12 col-md-12 col-lg-3">
                        <div class="invl-call-to-action-teste-channels-list-item">
                            <img class="img-fluid" src="public/images/channel-04.png">
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>